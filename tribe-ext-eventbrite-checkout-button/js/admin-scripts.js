jQuery(document).ready(function($) {
    ...
    function updateFieldIndexesAndLabels() {
        var rowCount = $eventbriteFieldsWrapper.find('.eventbrite-field-row').length;
        ...
        $eventbriteFieldsWrapper.find('.eventbrite-field-row').each(function(index) {
            ...
            // Keep at least one ticket option: only show remove control when there is more than one row.
            if (rowCount > 1) {
                $row.find('.remove-eventbrite-field-icon').show();
            } else {
                $row.find('.remove-eventbrite-field-icon').hide();
            }
        });
    }
    ...
    $('#add-new-eventbrite-field').on('click', function() {
        ...
        var newFieldHtml = `
            <div class="eventbrite-field-row">
                <button type="button" class="remove-eventbrite-field-icon" title="${eventbrite_ticket_button_vars.remove_button_text}" aria-label="${eventbrite_ticket_button_vars.remove_button_text}">
    ...
    $eventbriteFieldsWrapper.on('click', '.remove-eventbrite-field-icon', function() {
        if ($eventbriteFieldsWrapper.find('.eventbrite-field-row').length <= 1) {
            return;
        }
